...
order 3--10, and we measure a similar number, indicating that we are
...
overall numbers of detections.  Prefiltering may also prevent a
...
image processing, which will bias this ratio (see
Section~\ref{sec-bg}).  The observed ratios of 2--3 suggest that there
...
fitted sample respectively.  In the control case there is a minimum indicating that 
the the model is overfit while the fitted sample continues to decrease despite the 
increasing complexity.  For more discussion on this you can see Section $8.11$ in
\cite{TheSecondBestBook}.
...
even if the false positive rate is slightly higher.  We will continue working on this with more complicated simulations.
...
\hline
\multicolumn{5}{|c|}{Per Sensor MSE Comparison} \\
\hline
...
\end{tabular}
\caption{
We exampine the configs chosen based on the minimum MSE and compare them to the config of the
minimum number of false positives.  The sample here consists of 9 sensors per visit.  
N$_{Same}$ is the number of times the minimum MSE coincides with the minimum false positives.  N$_{<3}$
is the number of times the minimum false positive config has an MSE in the minimum 3.  $\Delta FP$
is the total change in false positive count using the minimum MSE config as compared to the sum of the minimum 
false positives per sensor.  max($\Delta FP$) is the maximum change in detected false positives on any sensor in that visit.
...
the coadd to the science exposure produces 1--2 orders of magnitude more scatter in the fit than at any other point. \label{tab-wcsrms}}
\end{table}
\clearpage
...
\caption{Here we show the comparison of detected false positive count to the analytic predition.
In all cases the blue circles are the prefilter case and the red squares are the postfilter case.  
The top row has had the counts corrected for the underestimate of the noise by the variance plane.  This is a 
1\% correction in the case of prefiltering and a 4\% correction to the variance in the postfiltering case.
In the bottom row we have corrected the predicted counts to coincide with the measured values.  From left to right
this is a 5\%, 3\%, 2\% correction to the variance respectively.  The shaded area is the 1-$\sigma$ confidence envelope
determined from Monte Carlo simulations assuming the analytic form of the false positive rate and a Gaussian distribution.}
