...
of this distrubion, the $\chi^2$ of these pixels normalized by the
...
spatial kenrel model, we assume that each of the kernel coefficients
...
having bad residuals after spatial modelling.  However in this
...
$K(u,v,x,y) = \sum_i a_i(x,y) K_i(u,v)$, we evaluate the solution at
the position of each {\tt KernelCandidate} and generate the metrics
defined above.  These are referred to as the SPATIAL kernel metrics.
Importantly, we interpolate this solution to the positions of the
...
and extrapolate to the full extent of the images.
