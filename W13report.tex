...
(XXX name these metrics), in order to assess the interpolation ability
...
{XXX are you sure this is correlation - I thought it was convolution
  and should be cross_circ}
...
%XXX K(x,y,mu,nu) is pretty confusing  - especially as it has an
%explicit sum in it for a number of the PSF detection steps (ie sum
%over mu,nu)
...
order, with the others modified by order//2 {XXX what is this?}.  Figure~\ref{basis}
...
%{XXX footnote should note be a * as it confuses the math...
...
{XXX is this true - Simon said it does vary from device to device}
...
drive the fit.
{XXX: is this just saying that the astrom fits work if you up the
  noise}
...
\item For non-deconvolved image subtraction, prefiltering performs as
  well as post filtering in terms of the number of false positives.
{XXX true}
...
XXX: Figure 13/14 should we subtract the number of random false
positives from the plots as we are looking for excess number of false
positives}
