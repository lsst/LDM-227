...
We
investigated the complexity of the basis functions used to model
...
kernel.  These metrics are shown to correlate with the numbers of false positives.  
We calculate a mean squared error metric that shows a minimum close to the configurations
that yielded the lowest number of false positives, and shows promise as a means
to algorithmically choose the optimal pipeline configuration.
...
3--12, depending upon seeing.  We measure a similar number, indicating that we are
able to achieve the systematic floor in false detection rate.  We
...
there to be less than 1 expected false detection per sensor (to
seeings as good as 0.6''), we must operate at a detection threshold of
5.5--sigma.
...
have the same seeing) for 5/9 sensors.  This provides an interesting test
...
the latter would be due to fluctuations in the background.  We note 2
...
12 expected, and 1 additonal false positive in the worst seeing case, compared to 3 expected.  These numbers roughly
double if the noise is underestaimted by 2\%.
...
background random Gaussian field.  We find that we can perform a 1--parameter correction to these curves that
yields both the same slope and zeropoints as the theoretical curves, with the exception of the deconvolution 
visit.  This correction involves scaling {\it down} the variance in the images by 2.5\%, 1.5\% and 1\% for 
v6866601, v6866602, and v6866603 respectively.  The data plotted with these corrections are shown
in the bottom row of Figure\ref{fig-fp_v_thresh}.  These results suggest our varaince estimates 
...
which suggest that our variance planes {\it underestaimte} the true variance.  However, this makes clear that 
...
\item At the 5--sigma detection threshold, for the range of seeings
  ...
  {\bf We are able to realize this theoretical floor}.  This function is steep;
  by increasing the detection threshold to 5.5 sigma this number may
  be lowered to less than 1 per sensor down to seeing of 0.6'' (Table~\ref{tab-fp}).  
  This translates to of order 2 * $10^6$
  per night, assuming 1000 visits / night.  
...
\item Our empirical negative to positive false detection ratio is 2--3 for prefiltering, 
  ...
  in postfiltering, although they vary from over-- to under--subtraction of the background (Table~\ref{tab-bestfp10}).
...
\item The measured false detection rate is seen to scale with detection
  ...
  into almost exact correspndence.  Optimization of the false detection vs. true source
...
\item The kernel it able to absorb, individually, random scatter in the astrometry (Figure~\ref{wcsrms})
  and offsets in the astrometry  (Figure~\ref{wcsshift}) of order 0.2''
  without a catastrohic increase in the number of false positives.  
...
\item Higher order measuremnts on filtered sources are not yet available,
...
\item Using the mean squared error of a control sample of {\tt KernelCandiates}
...
inputs to the coadd process, to exercise use of the coadd process and coadd repository.  As 
...
typically off the {\tt patch} image.  The TAN--SIP model is unable to perform adequately in this circumstance,
as it expands its polynomials around the CRPIX.  To get around this 
we tried re--fitting the coadd patches using a TAN--SIP solution that was solved from
scratch.  This worked fairly will part of the time,
yielding residuals in the 40--100 mas range, compared to the 100--300 mas previously.  However,
...
\end{tabular}
\caption{
We examine the configs chosen based on the minimum MSE and compare them to the config of the
minimum number of false positives.  The sample here consists of 9 sensors per visit.  
N$_{Same}$ is the number of times the minimum MSE coincides with the minimum in false positives.  N$_{<3}$
is the number of times the minimum false positive config is within the best 3 of MSE.  $\Delta FP$
is the total change in false positive count using the minimum MSE config as compared to the sum of the minimum 
...
\begin{figure}
  \centering 
  \subfloat[][0.6$^{\prime\prime}$ FWHM]{\includegraphics[width=1.0\textwidth]{figures/dfp_v_sigma0.60.eps}}
  \caption[]{ We plot here the change on the number of false positives
...
\begin{figure}
  \ContinuedFloat 
  \centering 
  \subfloat[][0.88$^{\prime\prime}$ FWHM]{\includegraphics[width=1.0\textwidth]{figures/dfp_v_sigma0.88.eps}}
  \caption[]{}
\end{figure} 
\clearpage
\begin{figure}
  \ContinuedFloat 
  \centering 
  \subfloat[][1.2$^{\prime\prime}$ FWHM]{\includegraphics[width=1.0\textwidth]{figures/dfp_v_sigma1.20.eps}}
  \caption[]{}
\end{figure} 
...
In the bottom row we undertaken a 1--parameter correction to align the predicted and measured values.  From left to right
this is requires a 5\%, 3\%, 2\% reduction of the image variance, respectively.  
